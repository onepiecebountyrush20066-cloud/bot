...
# إنشاء المجلدات على جهازك إن لم تكن موجودة
...
# ==================== تصدير وحفظ ملف الأعضاء على الجهاز ====================
def save_members_points_to_file():
    """تصدير بيانات الأعضاء ونقاطهم إلى ملف نصي داخل جهازك مباشرة"""
    try:
        conn = get_db()
        cursor = conn.cursor()
...
async def manual_export_members(client: Client, message: Message):
    ...
    await message.reply(f"✅ تم حفظ بيانات ونقاط <b>{total}</b> عضو بنجاح داخل جهازك في المسار:\n<code>{MEMBERS_EXPORT_PATH}</code>")
...
# ==================== رفع ملفات ULP وحفظها داخل الجهاز ====================
@app.on_message(filters.regex("^📤 رفع ملف ULP$") & filters.user(OWNER_IDS))
async def ask_file(client: Client, message: Message):
    await message.reply("📤 أرسل ملفات الـ <b>ULP</b> الآن لمعالجتها وإضافتها للكومبو وحفظها في جهازك...")
...
async def handle_large_document(client: Client, message: Message):
    try:
        ...
        # حفظ الملف المرفوع بشكل دائم في مجلد uploaded_ulp داخل الجهاز
        saved_file_path = os.path.join(UPLOAD_DIR, f"{uuid.uuid4().hex[:6]}_{file_name}")

        msg = await message.reply(f"⏳ جاري تحميل وحفظ الملف في جهازك: <b>{file_name}</b> ({file_size / 1024 / 1024:.2f} MB)...")
