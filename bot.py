...
DB_PATH = "/storage/emulated/0/combo-bot/combos.db"
...
async def start_handler(client: Client, message: Message):
    ...
    # تفريغ حالة المستخدم عند بداية الضغط على start
...
async def handle_large_document(client: Client, message: Message):
    try:
        ...
        temp_path = await message.download(file_name=f"temp_{message.id}_{file_name}")
...
async def process_domain_input(client: Client, message: Message):
    ...
    if message.text in [
        ...
        "💰 رصيدي ونقاطي", "🔗 رابط الإحالة", "📺 قنوات الإعلانات"
    ]:
        return

    # التأكد من وجود حالة تنتظر الإدخال قبل المعالجة لتفادي الرسائل العشوائية
    ...
    # فحص ومعاينة العدد المتاح بالموقع
    ...
    # بناء الأزرار الشفافة المتعددة للآلاف
    buttons_list = []
